...
namespace U8.Framework.IO
{
    ...
    public static class U8File
    {
        ...
        /// <returns>结果</returns>
        public static StreamReader GetStreamReader(string path, U8FileType fileType = U8FileType.Text, Encoding fileEncoding = null)
        {
        ...
        /// <returns>结果</returns>
        public static StreamWriter GetStreamWriter(string path, U8FileType fileType = U8FileType.Text, Encoding fileEncoding = null, FileMode fileMode = FileMode.Append, FileAccess fileAccess = FileAccess.Write)
        {
            if (fileEncoding == null)
            {
                fileEncoding = Encoding.Default;
        ...
        public static void BinaryWrite(object obj, string path, U8FileType fileType = U8FileType.Text, FileMode fileMode = FileMode.OpenOrCreate, FileAccess fileAccess = FileAccess.Write)
        {
            using (FileStream fileStream = new FileStream(path, fileMode, fileAccess))
            {
                BinaryFormatter formatter = new BinaryFormatter();

                if (fileType == U8FileType.Text)
                {
        ...
        /// <param name="fileAccess">文件访问方式</param>
        /// <returns>对象</returns>
        public static T BinaryRead<T>(string path, U8FileType fileType = U8FileType.Text, FileMode fileMode = FileMode.Open, FileAccess fileAccess = FileAccess.Read)
        {
            if (!File.Exists(path))
            {
                return default(T);
