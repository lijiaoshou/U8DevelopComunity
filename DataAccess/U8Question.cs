...
namespace DataAccess
{
    public class U8Question
    {
        ...
        //将问题设为知识库内容
        public static bool PushIntoKnowledge(string databaseConnectionString, string QuestionId, string userid)
        {
            string sql = string.Format(@"UPDATE dbo.QuestionTable SET InKnowledgeLib='1',UpdateTime='{0}',Modifiler='{1}'  WHERE id='{2}'", DateTime.Now, userid, QuestionId);
        ...
        public static List<Entity.U8Question> SearchQuestionList(string databaseConnectionString, int currentPage, int pageSize, string questionCategory, string knowledgeCategory, string userid, out int allCount)
        {
            ...
            if (questionCategory == "2" && knowledgeCategory == "2")
            {
                sql = sql.Replace("t.SubmitTime desc", "t.Popularity desc");
            }
            if (questionCategory == "2" && knowledgeCategory == "1")
            {
                sql = sql.Replace("t.SubmitTime desc", "t.UpdateTime desc");
            }
        ...
        //查出某个问题的所有回答
        public static string GetAnswers(string databaseConnectionString, string id, string userid)
        {
            string answerRegion = @"";
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    bool isAuthor = userid == U8Convert.TryToString(dt.Rows[0]["submiter"]) ? true : false;
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
        ...
        //找出问题提问者的id
        public static string GetUserIdByQuestionId(string databaseConnectionString, string id)
        {
            string sql = string.Format(@"
                                       SELECT Submiter FROM dbo.QuestionTable WHERE id='{0}'
                                       ", id);
            DataTable dt = new DataTable();
            U8Database.Fill(databaseConnectionString, sql, dt, null);
            if (dt != null && dt.Rows.Count > 0)
            {
                return U8Convert.TryToString(dt.Rows[0]["Submiter"]);
            }
            return string.Empty;
        }
